// Panel Components loader (ADR-0010). set = remount by id (new element +
// property-assigned props); clear = remove by id. Ops may arrive before the
// entry module loads; the custom element upgrade then wires the
// already-inserted element without losing props.
...
function loadPluginUIs() {
  fetch('/api/plugins').then(function (r) { return r.json(); }).then(function (b) {
    ...
    appendPre('plugin list failed: ' + e, 'message error');
  });
}

export { panelHost, applyPanel, loadPluginUIs };
