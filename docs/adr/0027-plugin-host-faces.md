...
- CLI 与 server 的 `-scheme` flag **彻底删除**（已评审定案，明确否决「保留但换实现」）：切换只有两个入口——交互式 `/agent config set defaultScheme=<name>`（REPL / Web 命令面，agent 插件的 `commands` 面）与 `config.json` 的 `defaultScheme`。由此产生的后果一并接受：一次性 `-turn` 模式**无法在启动时预选 scheme**，须先设 defaultScheme 或进 REPL 切换；README 快速开始中 4 处 `-scheme` 示例在 Z5 全部改写。
