...
工具执行前的 allow/ask/deny 由 provides `policy` 的 permission 插件实现；Agent Loop 在 callTool 前主动 `policy.decide`，ask 经 Host `agent.request` 回到 Render Medium。Host 星型路由只保留 closed 拒绝（ADR-0014）。
