...
2. **Host 横切面（临时保留特例，ADR-0030 Deferred）**  
   `cap=agent` 且 method ∈ `request` | `inject` | `confirm` → `handleAgentFromPlugin`
3. **`host.ensurePlugins`**（ADR-0023）→ 幂等按名挂载 + `dependsOn` 闭包
4. **`tools` 多提供方**（ADR-0018）→ `routeToolsFromPlugin`：  
   - `list`：合并已挂载 tools 提供方  
   - `call`：按 tool 名找 owner 后转发；owner 是自己则走 `callByPlugin` 绕过 self-block
5. **普通 Capability**：查 `provides[cap]` 找 owner  
   - 无 owner 或 owner == 调用方 → `capability_unavailable`  
   - `ensureAlive(owner)` → 换 `fwd-N` 写入 owner stdin  
   - 超时定时器到点 → 回 `timeout` 并删 pending
