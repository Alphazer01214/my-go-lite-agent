...
**SSE**：`serve.Subscribe` → `event: <topic>\ndata: {topic,data}\n\n`。Topic：`presentation|status|stream|panel|evt`——`evt` 是无 id 插件 evt 的通用中继（ADR-0034）；session choice 问答经它推送，并经 `/api/call to=session` 应答。慢消费者丢 `stream`。挂载时 PanelOps 从 `srv.Panels()` 种子回放。
...
| 测试 | 覆盖 |
|------|------|
| `server_test.go` | Shell/SDK、路径穿越、Plugin Graph 全目录、replay、choice.respond 通用 call 路由、Workspace 合并 |
