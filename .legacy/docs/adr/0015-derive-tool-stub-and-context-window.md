...
> Status: partially superseded by ADR-0028 —— 「自动压缩仍 deferred」已由 ADR-0028 推翻；Tool Result Stub、Context Window 观测与 `compactHint` 的部分继续有效（「provider token 对账 deferred」仍为 backlog）。
