...
Permission 裁决由 sandbox（provides `policy`）按规则 + severityPolicy 实现（ADR-0019）；规则源为进程级 `permissions.json` 与 Workspace 层覆盖。用户需要每个 Session 独立的粗粒度权限档：`ask` / `workspace_write` / `full_access`（原名 `read_only`，ADR-0034 改名），而不是只有全局 defaultAction。
...
取值：`ask` | `workspace_write` | `full_access`（旧持久化值 `read_only` 由 Normalize 兼容映射为 `ask`）。

### 裁决序（模式是默认档）

1. **显式 `permissions.json` rules**（deny > ask > allow）——可**放宽**模式（用户定案：显式 allow 覆盖模式）。
2. 无规则命中 → **Session `permissionMode` 档**：
   - `ask`（ADR-0034，原 `read_only`）：severity low → allow；medium/high → **ask**（经 session 插件 choice 弹卡，用户放行即 allow，不再静默 deny）。
...
| sandbox | decide 消费 `permissionMode`；ask 经 session 插件通用 choice（ADR-0034）；**最终 allow/deny 均应写入 Session Log `policy_decision`**。 |
